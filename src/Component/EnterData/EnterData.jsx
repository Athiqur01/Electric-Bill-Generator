 ...
 //Handle current reading
  ...
  <div className="flex flex-col md:flex-row justify-center gap-10 mt-8">
    <div className="flex flex-col items-center gap-2">
      <label className="text-xl font-semibold bg-purple-600 text-white py-2 px-4 rounded-lg shadow-sm">Billing Month:</label>
      <input 
        type="month" 
        name="date" 
        value={billingMonth} 
        className="text-center border border-purple-600 py-2 px-3 rounded-lg focus:ring-2 focus:ring-purple-300 transition" 
        required 
      />
    </div>

    <div className="flex flex-col items-center gap-2">
      <label className="text-xl font-semibold bg-purple-600 text-white py-2 px-4 rounded-lg shadow-sm">Due Date:</label>
      <input 
    ...
    <div className="flex flex-col items-center gap-2">
      <label className="text-xl font-semibold bg-purple-600 text-white py-2 px-4 rounded-lg shadow-sm">Issue No.:</label>
      <input 
        onChange={handleIssueNo} 
        type="text" 
        name="date" 
        className="text-center border border-purple-600 py-2 px-3 rounded-lg focus:ring-2 focus:ring-purple-300 transition" 
        required 
  ...
  {/* Column Headings */}
  <div className="overflow-x-auto mt-8 px-2 md:px-8 lg:px-2">
    <div className="grid grid-cols-8 gap-2 bg-gray-100 p-2 rounded-lg">
  ...
  {/* Data Input */}
  <div className="mt-2 space-y-2 px-2 md:px-8 lg:px-2">
    {subscriber2.map((subscriber, index) => (
      <form className="grid grid-cols-8 gap-2 items-center bg-white p-2 rounded-lg shadow-sm" key={subscriber?._id}>
        <input type="text" value={index + 1} className="text-center text-black bg-gray-50 rounded-md" readOnly />
        <input type="text" value={subscriber?.name} className="text-center text-black bg-gray-50 rounded-md" readOnly />
        <input type="text" value={subscriber?.designation} className="text-center bg-gray-50 rounded-md" readOnly />
        <input type="text" value={subscriber?.flatNo} className="text-center bg-gray-50 rounded-md" readOnly />
        <input type="text" value={subscriber?.reading} className="text-center border border-purple-600 rounded-md px-1" />

        <input
          type="text"
          onChange={(e) => handleInputChange(e, subscriber)}
          className="text-center border border-purple-600 rounded-md px-1"
        />

        <input type="text" value={bill[subscriber?.id]?.units} className="text-center bg-gray-50 rounded-md" readOnly />
        <input type="text" value={bill[subscriber?.id]?.calculatedBill} className="text-center bg-gray-50 rounded-md" readOnly />
      </form>
    ))}
  </div>
  ...
  <div className="flex justify-center mt-6">
    <button 
      ...
      type="submit" 
      className="bg-purple-600 text-white px-6 py-2 rounded-lg shadow-lg hover:bg-purple-700 transition"
    >
      Submit
    </button>
  </div>
